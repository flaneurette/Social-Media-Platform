...
<html>
	...
	?>
		...
		<div id="photo-editor-main">
			...
			<input type="button" value="Reset" onclick="filter.resetFilters('image-edit');" />
		...
		<div id="wrapper">
					...
					<div id="nav-center">
						...
						<div id="timeline-header" style="<?php if(isset($profile[0]["header"])) { echo "background:url('".$db->clean($host.$profile[0]["header"],'encode')."');"; } ?>background-size: cover;background-repeat:no-repeat;background-color: #000!important; <?php echo $profile[0]["headerfilter"];?>">
							<div id="photo-edit-icon-header" onclick="Social.show('photo-editor');"></div>
						</div>
							<div id="timeline-profile-picture-container"><div id="timeline-profile-picture" style="background:url('<?php echo $host . $db->clean($profile[0]["photo"],'encode');?>') !important; background-size: cover!important;background-color: #000!important; <?php echo $profile[0]["photofilter"];?>"></div>
							</div>
						...
						<div id="image-container"><div id="image" style="background:url('<?php echo $host . $db->clean($profile[0]["photo"],'encode');?>') !important; background-size: cover!important; background-color: #000!important;"></div></div>
						...
						<span><img class="timeline-icon" src="<?php echo $host ;?>resources/images/icons/comment.png" onclick="Social.createCommentForm('timeline-post-bar-<?php echo $i;?>','../postback/','<?php echo $db->clean($csrf,'encode');?>','<?php echo $db->intcast($timeline[$i]['tid']);?>','<?php echo $db->intcast($timeline[$i]['uid']);?>','<?php echo $db->intcast($profile[0]['id']);?>','<?php echo $db->clean($profile[0]["username"],'encode');?>');" /></span>
						<span class="tl-opt-num" id="heart<?php echo $i;?>-num"><?php echo $num_hearts;?></span><span><img class="timeline-icon" id="heart<?php echo $i;?>" src="<?php echo $host ;?>resources/images/icons/heart.png"  onclick="Social.timelineoptions('<?php echo $host;?>','heart','<?php echo $db->intcast($timeline[$i]['uid']);?>','<?php echo $db->intcast($timeline[$i]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>','heart<?php echo $i;?>')" /></span>
						<span class="tl-opt-num" id="star<?php echo $i;?>-num"><?php echo $num_stars;?></span><span><img class="timeline-icon" id="star<?php echo $i;?>" src="<?php echo $host ;?>resources/images/icons/star.png" onclick="Social.timelineoptions('<?php echo $host;?>','star','<?php echo $db->intcast($timeline[$i]['uid']);?>','<?php echo $db->intcast($timeline[$i]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>','star<?php echo $i;?>')"/></span>
						<span class="tl-opt-num"><?php echo $num_shares;?></span><span><img class="timeline-icon" src="<?php echo $host ;?>resources/images/icons/share.png" onclick="Social.timelineoptions('<?php echo $host;?>','share','<?php echo $db->intcast($timeline[$i]['uid']);?>','<?php echo $db->intcast($timeline[$i]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>')"/></span>
						<?php 
						if($profile[0]["id"] == $_SESSION['uid']) { 
						?>
						<span class="options-opt-icon"><img class="options-opt" onclick="Social.showTimelineOptions('timeline-options-box-<?php echo $i;?>');" src="<?php echo $host ;?>resources/images/icons/opt.png"/></span>
							<div id="timeline-options-box-<?php echo $i;?>" class="timeline-options-box">
								<img src="<?php echo $host ;?>resources/images/icons/close.png" class="timeline-options-box-close" onclick="Social.closeTimelineOptions('timeline-options-box-<?php echo $i;?>');"/>
								<ul>
						...
						<div id="timeline-post-bar-<?php echo $db->intcast($id);?>" class="profile-reply">
