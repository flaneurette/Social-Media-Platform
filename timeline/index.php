		...
		<div id="wrapper">
					...
					<div id="nav-center">
						...
						<div id="timeline-post-self">
							...
							<textarea name="post-message" id="post-message" style="display:none;"></textarea>
							<div id="style-bar">
						...
						$flaggedlist = $db->query("SELECT * FROM flagged where flaggedby = '".$db->intcast($uid)."' and hide = '1'");
						if(count($flaggedlist) >=1) {
							for($a=0;$a<count($flaggedlist);$a++) {
								array_push($flaggedid,$flaggedlist[$a]['postid']);
							}
						}
						...
						$selectfriends = $db->query("SELECT * FROM friends WHERE uid = '".$db->intcast($uid)."' and blk != '1'");
								...
								if(count($timeline_friends) >=1) { 
									if(!empty($timeline_friends)){ 
											for($l=0;$l<count($timeline_friends);$l++) {
												...
												if(!in_array($timeline_friends[$l]['tid'],$flaggedid)) {
						...
						for($k=0; $k < count($timelines); $k++) { 
							...
							?>
							<div class="timeline-post" id="timeline-post-<?php echo $k;?>">
							...
							<a href="<?php echo $host . '@' .ucfirst($db->clean($timelines[$k]['username'],'encode'));?>/<?php echo $db->intcast($timelines[$k]['uid']);?>/status/<?php echo $db->intcast($timelines[$k]['tid']);?>/">
								<p class="twig">
									<?php 
								...
								<div class="timeline-options">
									<span><img class="timeline-icon" src="/resources/images/icons/comment.png" onclick="Social.createCommentForm('timeline-post-bar-<?php echo $k;?>','../postback/','<?php echo $db->clean($csrf,'encode');?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->intcast($timelines[$k]['uid']);?>', '<?php echo $db->intcast($timelines[$k]['id']);?>', '<?php echo $db->clean($timelines[$k]['username'],'encode');?>');" /></span>
									<span class="tl-opt-num" id="heart<?php echo $k;?>-num"><?php echo $num_hearts;?></span><span><img class="timeline-icon" id="heart<?php echo $k;?>" src="/resources/images/icons/heart.png"  onclick="Social.timelineoptions('<?php echo $host;?>','heart','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>','heart<?php echo $k;?>')" /></span>
									<span class="tl-opt-num" id="star<?php echo $k;?>-num"><?php echo $num_stars;?></span><span><img class="timeline-icon" id="star<?php echo $k;?>" src="/resources/images/icons/star.png" onclick="Social.timelineoptions('<?php echo $host;?>','star','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>','star<?php echo $k;?>')"/></span>
									<span class="tl-opt-num"><?php echo $num_shares;?></span><span><img class="timeline-icon" src="/resources/images/icons/share.png" onclick="Social.timelineoptions('<?php echo $host;?>','share','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>')"/></span>
								<?php 
								...
								<span class="options-opt-icon"><img class="options-opt-timeline" onclick="Social.showTimelineOptions('timeline-options-box-<?php echo $k;?>');" src="<?php echo $host ;?>resources/images/icons/opt.png"/></span>
									<div id="timeline-options-box-<?php echo $k;?>" class="timeline-options-box">
										<img src="/resources/images/icons/close.png" class="timeline-options-box-close" onclick="Social.closeTimelineOptions('timeline-options-box-<?php echo $k;?>');"/>
										<ul>
										<li onclick="Social.timelineoptions('<?php echo $host;?>','delete','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>')"><span class="timeline-options-box-icon-block"></span>delete post</li>
										</ul>
									</div>
								<?php } else { ?>
								<span class="options-opt-icon"><img class="options-opt-timeline" onclick="Social.showTimelineOptions('timeline-options-box-<?php echo $k;?>');" src="<?php echo $host ;?>resources/images/icons/opt.png"/></span>
									<div id="timeline-options-box-<?php echo $k;?>" class="timeline-options-box">
										<img src="/resources/images/icons/close.png" class="timeline-options-box-close" onclick="Social.closeTimelineOptions('timeline-options-box-<?php echo $k;?>');"/>
										<ul>
										<li onclick="Social.timelineoptions('<?php echo $host;?>','hide','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>'); Social.hide('timeline-post-<?php echo $k;?>')"><span class="timeline-options-box-icon-hide"></span>hide this post</li>
										<li onclick="Social.timelineoptions('<?php echo $host;?>','flag','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>'); Social.hide('timeline-post-<?php echo $k;?>')"><span class="timeline-options-box-icon-flag"></span>flag this post</li>
										<li onclick="Social.timelineoptions('<?php echo $host;?>','block','<?php echo $db->intcast($timelines[$k]['uid']);?>','<?php echo $db->intcast($timelines[$k]['tid']);?>','<?php echo $db->clean($csrf,'encode');?>'); Social.hide('timeline-post-<?php echo $k;?>')"><span class="timeline-options-box-icon-block"></span>block this user</li>
										</ul>
									</div>
								<?php } ?>
